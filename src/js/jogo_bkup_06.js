		...
		function i_esquerda(){
			...
			if(L1 > 0){
				...
				if(detectar_bloco === false){
					...
					//	.se estiver acima do ch�o detecta solo
					/*
	...
	if((tecla === 38 || tecla === 0 || tecla === "v") && personagem.getAttribute("class").search("cima_i") === -1 && personagem.getAttribute("class").search("baixo_i") === -1){		
		...
		//"use strict";
			...
			// chegou ao solo
				...
				if(blocos[0][0].style.background.search("/src/images/activo.png") != -1 && personagem.getAttribute("class").search("baixo_i") != -1){
					// .quedas
...
/* LOG DE ERROS
	...
	- SALTAR = 1  impede a troca de variaveis , aumenta performance e detec��o do jogo
