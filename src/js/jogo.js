...
// Variaveis movimentos responsaveis
	...
	//"use strict";
		...
		per_back_esq = "url(/imagens/personagem_0_esq.png)";
		per_back_dir = "url(/imagens/personagem_0_dir.png)";
		...
		if(cima_i != -1){
			...
			if(estado_som === -1){
				...
				// se o valor é -40 o que significa , detecção de mola
		...
		if(dir_i != -1 && colisao != "direita"){
			...
			// se o left PER é menor que width do Jogo
		...
		if(e_length != 0){
			...
			for(d=0 ; d < e_length; d++){
				...
				if(T1-T3 > -36 && T1-T3 < -28 && R1-R3 < 30 && L1-L3 > -30 ){
					// se o top nao estiver ajustado fá-lo-à
					...
					// se estiver no mesmo bloco simplesmente continua a circular sem detecções
					else{
						det_bloco = "ajustado";
					}
				...
				// se depois de todas as verificações nao detectar pedra determina false e inicia descida
				else{
					d_pedras = 0;
				}
		...
		// se não existirem pedras envia sinal
		else{
			d_pedras = 0;
			colisao = 0;
		}
		...
		for(f=0 ; f < f_length; f++){
				...
				if(T1 != (T4 - 35)){
					...
					if(bloco_auxiliar_id === "aux_verde"){
						...
						if(E4.style.background.search("verde") === -1){
							E4.style.background = "url(\"/imagens/verde.png\")";
			...
			// se depois de todas as verificações nao detectar bloco determina false e inicia descida
			else{
				det_bloco = false;
			}
		}*/
		...
		for(b=0 ; b < b_length; b++){
			...
			if(T1-T2 > -36 && T1-T2 < -28 && R1-R2 < 30 && L1-L2 > -30 ){
				...
				if(T1 != (T2 - 35)){
					...
					// Super-Importante
					// estes codigos estão aqui para que sejam lidos, apenas se houver um primeiro contacto com o objecto em questão
						
					// verificação de sequencia
					caixa_azul = E2.style.background.search("/imagens/inactivo.png");
					...
					caixa_laranja = E2.style.background.search("/imagens/activo.png");						
					...
					// se não é o primeiro e o anterior NÃO esta activo
					else if(b > 0 && id_bloco_ant === -1){
						ligar_bloco = false;
					}
					// se não é o primeiro e o anterior esta activo
					...
					//sequencia correcta
						...
						if(caixa_azul != -1 && caixa_dourada === -1 && caixa_vermelha === -1 && caixa_laranja_3 === -1){
							// .background
							E2.style.background = "url(\"/imagens/activo.png\") #ffffff";
							// .border
						...
						// bloco vermelho
							...
							if(E2.style.background.search("/imagens/vermelho.png") === -1){
								// .background
								E2.style.background = "url(\"/imagens/vermelho.png\") #ffffff";
								// .border
								E2.style.border = "1px dashed #ffffff";
							}
							// se não estiver em "cima_i" , fá-lo-á
							// repor valores de origem
							valor_cima = -54;
							personagem.setAttribute("class",personagem.getAttribute("class").replace("cima_p","cima_i"));
							det_bloco = false;
						...
						// bloco dourado
							...
							ultimo_bloco = 1;
							// .background
							E2.style.background = "url(\"/imagens/dourado.png\") #ffffff";
							// .border
							E2.style.border = "1px dashed #ffffff";
					...
					// se metade dos blocos estiverem concluidos dá sinal
