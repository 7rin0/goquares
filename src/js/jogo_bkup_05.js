...
function loading_conteudos(){
	...
	// Array de src/images
	load_src/images = ["personagem_inicio", "fundo","fundo_2","fundo_3","fundo_4","inactivo", "activo", "vermelho", "dourado","personagem_0_esq","personagem_0_dir"];

	// Loop de src/images
	for(a=0; a < load_src/images.length; a++){
		loading_imagem = new Image();
		loading_imagem.src = "/src/images/"+load_src/images[a]+".png";
	}
...
"use strict";
	...
	// Tempo
		...
		// Temporizador, fun��o que repetir�
...
"use strict";
	...
	"use strict";
		...
		if(parseInt(document.getElementById("janela_jogo").style.height)-parseInt(personagem.style.height)-30 <= parseInt(personagem.style.top)){
			...
			personagem.setAttribute("class"," esq_p dir_p cima_p accao");
	...
	document.addEventListener('keydown',function teclaPresa(event) {
		...
		switch (event.keyCode) {
			...
			case 38:
				...
				if(personagem.getAttribute("class").search("cima_i") === -1 && personagem.getAttribute("class").search("gravidade") === -1){
...
// Variaveis movimentos responsaveis
	...
	for (e=0 ; e < e_length; e++){
		...
		// Receber valores dos auxiliares
...
// #### Se tecla esquerda for premida 
if(tecla === 37 && personagem.getAttribute("class").search("esq_i") === -1 && personagem.getAttribute("class").search("dir_i") === -1){
	// redefinir class
	personagem.setAttribute("class",personagem.getAttribute("class").replace("esq_p","esq_i"));
	// se j� existe nao muda
	if(personagem.style.background.search("personagem_0_esq.png") === -1){
		personagem.style.background = "url(/src/images/personagem_0_esq.png)";
	}

	function i_esquerda(){		
		//	altura e esquerda da personagem
		T1 = parseInt(personagem.style.top,10);  
		L1 = parseInt(personagem.style.left,10); 
		detectar_bloco = false;
		
	//	objecto move esquerda
		if(L1 > 0){
			//# DETECTAR PEDRAS
			...
			// .dimens�es pedra
				...
				if(d<f_length){
					...
					var bloco_auxiliar = auxiliares[d][0],
						bloco_auxiliar_id = bloco_auxiliar.getAttribute("id"),
						bloco_auxiliar_class = bloco_auxiliar.getAttribute(
						"class");
				// . se encontrar AUXILIAR pousar� em cima do mesma
					if(auxiliares[d][0].style.display != "none"&& bloco_auxiliar_class.search("auxiliar") != -1 && T1-T4 >= -30 && T1-T4 <= 50 &&  L1-R4 >= -6 && L1-R4 <= 6 && janela_jogo.getAttribute("class").search("inicio") != -1 ){
						detectar_bloco = true;
				...
				// 3 pixeis a mais dos borders
				if(T1-T3 >= -26 && T1-T3 <= 46 && L1-R3 >= -9 && L1-R3 <= 9 && pedras[d][0].getAttribute("class").search("dinamite") === -1 && pedras[d][0].style.display != "none"){
				...
				// se for dinamite
					else if(pedras[d][0].style.display != "none" && T1-T3 >= -35 && T1-T3 <= 46 && L1-R3 >= -9 && L1-R3 <= 9  && pedras[d][0].getAttribute("class").search("dinamite") != -1){
						
							//se o background nao � dinamitico
							if(pedras[d][0].style.background.search("dinamite") === -1){
								
								pedras[d][0].setAttribute("class",pedras[d][0].getAttribute("class").replace("dinamite",""));
								
								var bomba_bips = 0,
									pedra_tnt = pedras[d][0];
								e_num_f("DANGER","perigo");
							
								
								// BOMBA
								cenario_bomba(bomba_bips,estado_som,pedra_tnt);
							}
						break;
					}
			...
			if(detectar_bloco === false){
				...
				//	iniciar detec��o
				if(T1<Tbase && personagem.getAttribute("class").search("cima_i") === -1 && personagem.getAttribute("class").search("gravidade") === -1 && personagem.getAttribute("class").search("dir_i") === -1){
					//	detectar blocos
						det_obj_pers( 0 , 1 );
				}

				//	fundo move direita
				if(parseInt(back_jogo.style.left,10) < 0 && L1 < parseInt(back_jogo.style.width,10)-300  && personagem.getAttribute("class").search("esq_p") === -1){
...
// #### Se tecla direita for premida 
if(tecla === 39 && personagem.getAttribute("class").search("dir_i") === -1 && personagem.getAttribute("class").search("esq_i") === -1){
	// redefinir class
	...
	if(personagem.style.background.search("personagem_0_dir.png") === -1){
		personagem.style.background = "url(/src/images/personagem_0_dir.png)";
	}
	...
	function i_direita(){		
		...
		if(L1 < parseInt(back_jogo.style.width,10)-parseInt(personagem.style.width,10)){
			...
			// .dimens�es pedra
				...
				if(d<f_length){
					...
					var bloco_auxiliar = auxiliares[d][0],
						...
						"class");
				// . se encontrar AUXILIAR pousar� em cima do mesma
					if(bloco_auxiliar_class.search("auxiliar") != -1 && T1-T4 >= -30&& T1-T4 <= 50 &&  R1-L4 >= -6 && R1-L4 <= 6 && janela_jogo.getAttribute("class").search("inicio") != -1 && auxiliares[d][0].style.display != "none"){
						detectar_bloco = true;
				...
				// se for pedra
				if(T1-T3 >= -26 && T1-T3 <= 46 && R1-L3 >= -9 && R1-L3 <= 9 && pedras[d][0].getAttribute("class").search("dinamite") === -1 && pedras[d][0].style.display != "none"){
					detectar_bloco = true;
					personagem.style.left = L1-(R1-L3)+"px";
					break;
				}
				// se entrar dentro da pedra 
				else if(T1-T3 >= -26 && T1-T3 <= 46 && R1-L3 >= -9 && R1-L3 <= 30 && pedras[d][0].getAttribute("class").search("dinamite") === -1 && pedras[d][0].style.display != "none"){
					detectar_bloco = true;
					personagem.style.left = L1-(R1-L3)+"px";
					break;
				}
				...
				// se for dinamite
				else if(T1-T3 >= -35 && T1-T3 <= 46 && R1-L3 >= -9 && R1-L3 <= 9 && pedras[d][0].getAttribute("class").search("dinamite") != -1 && pedras[d][0].style.display != "none"){
			...
			if(detectar_bloco === false){
				personagem.style.left = L1+6+"px";
				//	iniciar detec��o
				if(T1 < Tbase && personagem.getAttribute("class").search("cima_i") === -1 && personagem.getAttribute("class").search("gravidade") === -1 && personagem.getAttribute("class").search("esq_i") === -1){
				//	detectar blocos
					det_obj_pers( 0 , 1 );
				}
...
if(((tecla === 38 || tecla === "v") && personagem.getAttribute("class").search("cima_i") === -1 && personagem.getAttribute("class").search("gravidade") === -1) || ((tecla === 37 || tecla === 39 || tecla === 0) && personagem.getAttribute("class").search("cima_i") === -1 && personagem.getAttribute("class").search("gravidade") === -1) && valor != "nada"){
	
	//	###	Se for determinada tecla
	if((tecla === 38 || tecla === "v") && personagem.getAttribute("class").search("gravidade") === -1 ){
	// .personagem no ar
	...
	//"use strict";
		...
		// se dois eventos se cruzam
		if(personagem.getAttribute("class").search("gravidade") != -1 && personagem.getAttribute("class").search("cima_i") != -1){
			personagem.setAttribute("class",personagem.getAttribute("class").replace("cima_i","cima_p"));
			personagem.setAttribute("class",personagem.getAttribute("class").replace("gravidade",""));
		...
		// IMPORTANTE // .personagem a subir ...
		if(saltar <= 0 && (tecla === 38 || tecla === "v") && personagem.getAttribute("class").search("cima_i") != -1 && personagem.getAttribute("class").length != 0){
			...
			// .dimens�es pedra
				T3 = pedras[d][1]; //top pedra
				var B3 = pedras[d][3]; //bottom pedra
				R3 = pedras[d][2]; //direita pedra
				L3 = pedras[d][4]; //esquerda pedra
			// . se encontrar pedra pousar� em cima do mesma
				if(T1-T3 >= 45 && T1-T3 <= 50 && R1-R3 <= 30 && L1-L3 >= -30 && janela_jogo.getAttribute("class").search("inicio") != -1 ){
					detectar_bloco = true;
					// melhorar o top
					personagem.style.top = B3+"px";
					
				// se for dinamite
				if(pedras[d][0].getAttribute("class").search("dinamite") != -1){
						detectar_bloco = false;
						//se o background nao � dinamitico
						if(pedras[d][0].style.background.search("dinamite") === -1){
			...
			// se detectar pedra der "true" para tudo isto e a pesquisa inclusive
		...
		// IMPORTANTE 
		// .personagem a descer ...
		else if(saltar > 0 && T1 < Tbase && personagem.getAttribute("class").length != 0 && janela_jogo.getAttribute("class").search("inicio") != -1){
		detectar_bloco = false;
		...
		if(detectar_bloco === false){
			...
			// .dimens�es pedra
				...
				L3 = pedras[d][4]; //esquerda pedra
			// . se encontrar pedra pousar� em cima do mesma
				if(T1-T3 <= -35 && T1-T3 >= -40 && R1-R3 <= 25 && L1-L3 >= -25 && janela_jogo.getAttribute("class").search("inicio") != -1 ){
					// Ajustar personagem aos pedras
					if(T1-T3 != 0){
						var top_restante = 35+T1-T3;
						personagem.style.top = T1 - top_restante+"px";
					}
					detectar_bloco = true;
				}
			// se detectar pedra der "true" para tudo isto e a pesquisa inclusive
		...
		if(detectar_bloco === false){
			...
			// .dimens�es auxiliar
				...
				var bloco_auxiliar = auxiliares[f][0],
					bloco_auxiliar_id = bloco_auxiliar.getAttribute("id"),
					bloco_auxiliar_class = bloco_auxiliar.getAttribute(
					"class");
			// . se encontrar AUXILIAR pousar� em cima do mesma
				if(bloco_auxiliar_class.search("auxiliar") != -1 && T1-T4 >= -35 && T1-T4 <= -25 &&  L1-L4 >= -25 && R1-R4 <= 25 && janela_jogo.getAttribute("class").search("inicio") != -1 ){

					// Ajustar personagem aos auxiliares
					if(T1-T4 != 0){
						var top_restante = 35+T1-T4;
						personagem.style.top = T1 - top_restante+"px";
					}
					if(bloco_auxiliar_id === "aux_verde"){
						if(bloco_auxiliar_class.search("activo") === -1){
							if(estado_som === -1){
								som_tic_tac_verde.play();
							}
							var i_limite_som = setTimeout(function limite_som(){
								som_tic_tac_verde.stop();
								clearInterval(i_limite_som);
							},1000);
						}
					
						if(bloco_auxiliar.style.background.search("verde") === -1){
							bloco_auxiliar.style.background = "url(\"/src/images/verde.png\")";
					...
					// AUXILIAR VERMELHO, EXCELENTE
					else if(bloco_auxiliar_id === "aux_vermelho" && bloco_auxiliar.style.display != "none"){
					// Ajustar personagem aos auxiliares
					if(T1-T4 != 0){
						var top_restante = 35+(T1-T4);
							personagem.style.top = T1 - top_restante+"px";
			...
			// se detectar auxiliar der "true" para tudo isto e a pesquisa inclusive
				if(detectar_bloco === true){
					if(tecla === 38 || tecla === "v"){
					// parar procura
						personagem.setAttribute("class",personagem.getAttribute("class").replace("cima_i","cima_p"));
		...
		if(detectar_bloco === false){
			...
			// .dimens�es bloco
				...
				L2 = blocos[b][4]; //esquerda bloco
					...
					// se concluir metade dos blocos o vermelho auxiliar � activo
			...
			// . se encontrar bloco pousar� em cima do mesmo
				if(T1-T2 <= -35 && T1-T2 >= -40 && R1-R2 <= 25 && L1-L2 >= -25 && janela_jogo.getAttribute("class").search("inicio") != -1){
					
					// Ajustar personagem aos blocos
					if(T1-T2 != 0){
						var top_restante = 35+T1-T2;
						personagem.style.top = T1 - top_restante+"px";
					}
					
					var caixa_azul = blocos[b][0].style.background.search("/src/images/inactivo.png"),
						...
						caixa_laranja = blocos[b][0].style.background.search("/src/images/activo.png"),
					...
					// se � o segundo ou outro que n�o esteja activo
						...
						if(id_bloco_ant != -1 && b_activo === -1){
							...
							else if(caixa_laranja_2 != -1 && (tecla != 0 ||  personagem.getAttribute("class").search("gravidade") != -1)){
								blocos[b][0].setAttribute("class",blocos[b][0].getAttribute("class").replace("laranja_2","laranja_1"));
								var ligar_bloco = true;

								// iniciar efeito
								e_num_f(id_bloco+1);
							}						else if(caixa_laranja_1 != -1 && (tecla != 0 ||  personagem.getAttribute("class").search("gravidade") != -1)){
				...
				// se:LARANJA 3
					...
					//detectado bloco
						detectar_bloco = true;
					// .background
						blocos[b][0].style.background = "url(\"/src/images/laranja_3.png\") #ffffff";
					// .border
					...
					//detectado bloco
						detectar_bloco = true;
					// .background
						blocos[b][0].style.background = "url(\"/src/images/laranja_2.png\") #ffffff";
					// .border
						blocos[b][0].style.border = "1px dashed #ffffff";
					// .pontos
					...
					//detectado bloco
						detectar_bloco = true;
					// .background
						blocos[b][0].style.background = "url(\"/src/images/activo.png\") #ffffff";
					// .border
						blocos[b][0].style.border = "1px dashed #ffffff";
					// .pontos
				...
				// se:AZUIS , se os quadradinhos s�o azuis padrao
				...
				// se:VERMELHO , se o quadradinho � vermelho
					else if(caixa_vermelha != -1 && ligar_bloco === true){
						if(blocos[b][0].style.background.search("/src/images/vermelho.png") === -1){
					// .background
							blocos[b][0].style.background = "url(\"/src/images/vermelho.png\") #ffffff";
					// .border
							blocos[b][0].style.border = "1px dashed #ffffff";
						}
					//detectado bloco
					...
					// se:DOURADO , se o quadradinho � dourado
					else if(caixa_laranja === -1 && caixa_dourada != -1 && ligar_bloco === true){
						//detectado bloco
						detectar_bloco = true;
						// .background
						blocos[b][0].style.background = "url(\"/src/images/dourado.png\") #ffffff";
						// .border
						blocos[b][0].style.border = "1px dashed #ffffff";
						//.som
						if(estado_som === -1 || estado_som === null){
				...
				} // fim da condi��o de blocos
			} // fim do for loops
		}
			if(detectar_bloco === false){
				if(tecla === 0 && personagem.getAttribute("class").search("gravidade") === -1){
					personagem.setAttribute("class",personagem.getAttribute("class")+" gravidade ");
				}
				personagem.style.top = T1 + 5 +"px";
			}
		} // fim da condi��o " saltar > 0 && T1 < Tbase "
		...
		// T1 > Tbase "
			...
			if(blocos[0][0].style.background.search("/src/images/activo.png") != -1 && (tecla === 38 || tecla === "v" || tecla === 0) && personagem.getAttribute("class").length != 0){
				// .quedas
...
/* LOG DE ERROS
	...
	detectar_bloco enviar false e detectar nas outras se � valido ou nao
